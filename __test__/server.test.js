...
const users = require('../src/auth/model/users.model')

const token = jwt.sign({ username: 'haha' }, process.env.SECRET || 'hamza');

beforeAll(async () => {
    await db.sync();
    await users.create({ username: 'haha', password: 'password', role: 'manager' })
});
...
const userData = {
    testUser: { username: 'haha', password: 'password', role: 'manager' }
}
